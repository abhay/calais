...
describe Calais::Client, ".new" do
  it "accepts arguments as a hash" do
    ...
    lambda { client = Calais::Client.new(:content => SAMPLE_DOCUMENT) }.should_not raise_error(ArgumentError)
  ...
  it "should not accept unknown attributes" do
    lambda { Calais::Client.new(:monkey => "monkey") }.should raise_error(NoMethodError)
  end
end

describe Calais, ".process_document" do
  it "returns a Calais::Response" do
    response = Calais.process_document(:content => SAMPLE_DOCUMENT, :content_type => :xml)
    response.should_not be_nil
    response.should be_a_kind_of(Calais::Response)
  end

  it "returns a Calais::Response (with relationships)" do
    response = Calais.process_document(:content => File.read(File.join(File.dirname(__FILE__), 'fixtures', 'bicycles_austrailia.xml')), :content_type => :xml)
    response.should_not be_nil
    response.should be_a_kind_of(Calais::Response)
  end
end
...
describe Calais::Client, ".call" do
  before(:all) do
    @client = Calais::Client.new(:content => SAMPLE_DOCUMENT)
  end
...
describe Calais::Client, ".params_xml" do
  it "returns an xml encoded string" do
    client = Calais::Client.new(:content => SAMPLE_DOCUMENT, :content_type => :xml)
